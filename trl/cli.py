...
from .scripts.rlsc import make_parser as make_rlsc_parser
...
def main():
    ...
    elif args.command == "rlsc":
        # Get the default args for the launch command
        rlsc_training_script = resources.files("trl.scripts").joinpath("rlsc.py")
        args = launch_command_parser().parse_args([str(rlsc_training_script)])

        # Feed the args to the launch command
        args.training_script_args = sys.argv[2:]  # remove "trl" and "rlsc"
        launch_command(args)  # launch training
